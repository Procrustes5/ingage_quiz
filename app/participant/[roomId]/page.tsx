...
export default function Participant() {
  ...
  const [roomId, setRoomId] = useState<string>('1'); // Change '1' to the appropriate room ID
        ...
        {questions.map((question, index) => (
          <div key={question.id} className="w-full max-w-md">
            <div className="text-center mb-4">
              <h2 className="text-3xl font-bold text-foreground">{question.text}</h2>
            </div>
            <div className="grid grid-cols-2 gap-4">
              {question.options.map((option, idx) => (
                <button key={idx} className="bg-background border-2 border-[#d42f2f] rounded-lg py-4 px-6 text-xl font-medium text-foreground hover:bg-[#d42f2f] hover:text-primary-foreground transition-colors">
